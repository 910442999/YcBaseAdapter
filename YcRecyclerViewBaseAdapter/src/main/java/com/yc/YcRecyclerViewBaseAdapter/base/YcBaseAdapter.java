...
public abstract class YcBaseAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private static final int TYPE_BASE_HEADER_VIEW = 200000; //是父布局的 header
    ...
    public YcBaseAdapter(Context context, List<T> datas) {
    ...
    public YcBaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (mHeaderViews.get(viewType) != null) {
            return YcBaseViewHolder.create(mHeaderViews.get(viewType));
        }
        ...
        switch (viewType) {
            case TYPE_FOOTER_VIEW:
                ...
                viewHolder = YcBaseViewHolder.create(mFooterLayout);
                break;
            case TYPE_EMPTY_VIEW:
                viewHolder = YcBaseViewHolder.create(mEmptyLayout);
                break;
            case TYPE_NODATA_VIEW:
                viewHolder = YcBaseViewHolder.create(new View(mContext));
                break;
    ...
    public int getItemViewType(int position) {
        if (mDatas.isEmpty()) {
            if (mEmptyLayout != null && !isRemoveEmptyView) {
    ...
    public int getFooterViewCount() {
        return mLoadMoreEnable && !mDatas.isEmpty() ? 1 : 0;
    }
